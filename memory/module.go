...
// Edges is everything memory touches in the world. Real implementations are
// wired in cmd/kasi/main.go; simulated twins live in this package (docs/12).
type Edges struct {
	Clock runtime.Clock
}

// Module bundles durable facts the agent remembers across tasks (feature-memory.md) (docs/01).
func Module(e Edges) *runtime.Module {
	mod := runtime.NewModule("memory", Model{}, e)

	registerRemember(mod)
	registerForget(mod)
	return mod
}
