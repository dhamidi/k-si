...
- **[htmlc](https://github.com/dhamidi/htmlc)** — renders Vue Single-File-Component
  (`.vue`) syntax *server-side in Go* with no JS runtime. Components receive props
  from Go structs and evaluate a JS-compatible expression subset; directives
  (`v-if`, `v-for`, `:attr`, slots, scoped styles) work as authored. We use
  `RenderPage` for full pages and `RenderFragment` for the HTML fragments Turbo
  swaps in.
...
- **html.sh** — semantic-HTML authoring conventions. käsi's templates follow
  html.sh's guidance for correct, accessible, semantic markup (proper elements,
  global attributes, forms, tables) so the htmlc components render clean HTML.
  *(Assumption to confirm: html.sh is treated here as the authoring/reference
  convention layer rather than a runtime dependency. If it is instead a concrete
  Go library, its integration slots in beside htmlc without changing the rest of
  this design.)*
...
1. **Setup.** Configure the Fastmail account (store its API token as a secret),
   the `kasi.decode.ee` domain settings, and the sender allow-list
   ([04](./04-email.md)).
...
- **Writes** are turned into **runtime messages** fed to the reducer — e.g.
  submitting the completion page emits `task.done`; saving a route emits
  `route.updated`. This keeps every state change logged and replayable
  ([01](./01-architecture.md), [03](./03-persistence.md)); the UI never mutates
  the model directly.
...
- **Small and semantic.** Few pages, semantic HTML (html.sh conventions),
  accessible by default. The UI should feel like a settings panel, not an
  application.
