...
The model itself is never persisted except as an optional snapshot
(optimisation only). So on a fresh start: replay the log to rebuild the model,
which points into the content tables for the heavy data.
...
CREATE TABLE message_log (
  ...
  tag        TEXT    NOT NULL,             -- e.g. "email.received"
...
- **Append-only.** Rows are never updated or deleted (except by compaction after
  a snapshot, and even then only rows the snapshot subsumes).
...
### `snapshot` — replay optimisation

```sql
CREATE TABLE snapshot (
  upto_id    INTEGER NOT NULL,   -- log offset this snapshot reflects
  model      BLOB    NOT NULL,   -- serialised model
  created_at TEXT    NOT NULL
);
```

Load the newest snapshot, then replay `message_log WHERE id > upto_id`. Snapshots
are disposable: deleting them only slows startup. A snapshot must round-trip the
model exactly.
...
Fastmail delivery lands raw MIME here first (see [04](./04-email.md)); *then* an
`email.received` runtime message references the row. Storing raw bytes means we
can re-parse with improved code later and never lose fidelity.
...
a send effect transmits it via JMAP and emits `email.sent`, whose handler marks
it `sent`. This makes sending **crash-safe and idempotent**: after a restart a
reconciliation subscription re-emits `send-email` for every still-`pending` row
(see *Reconciliation* below). The `Message-ID` is generated before send so a
duplicate send is detectable.
...
replayed, snapshotted, and inspected freely — never contains plaintext secrets,
and the secrets file can be given tighter permissions and a separate backup
policy. Its schema and the `secret://` resolution model are in
[06](./06-secrets.md).
